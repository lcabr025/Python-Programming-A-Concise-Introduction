...
# capture the arguments from the command line at position [1] & position [2]
...
# For every line we read, we calculate the length of the line, and write that to a file
for line in infile.readlines():
    line = line.strip("\n")
    count = len(line)
    outfile.write( str(count) + '\n' )

# Everything we open, we must close!
infile.close()
outfile.close()
